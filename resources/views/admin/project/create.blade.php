                ...
                <div class="card">
                    <div class="card-body offset-1 col-lg-10">
                        <div class="basic-form">
                            <form role="form" method="POST" action="{{ route('store-project') }}" enctype="multipart/form-data">
                                @csrf
                                @method('post')
                                ...
                                <div class="form-group{{ $errors->has('project_category_id') ? ' has-error' : '' }}">
                                    ...
                                    <select class="form-control w-30" name="project_category_id" value="{{ old('project_category_id') }}" id="selectType">
                                        ...
                                        @foreach ($activedProjectCategories as $item)
                                            <option value="{{$item['id']}}" {{ old('project_category_id') == $item['id'] ? 'selected' : '' }}>{{$item['title_en']}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <a type="button" href="{{ route('list-project') }}" class="btn btn-default btn-outline"><i class="ti-back-left icon-black"></i>&nbsp;&nbsp;Quay lại</a>
